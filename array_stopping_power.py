...
def check_stop(energy, stppow, rho, distance_travelled_in_array):
    ...
    equiv_R = np.floor(R/5) #One unit in the matrix corresponds to a real-world distance of 50mm or 5 cm

    stopping_prob = np.exp(-distance_travelled_in_array/equiv_R)

    chance = np.random.random()

    if chance <= stopping_prob:
        return True
    else:
        return False